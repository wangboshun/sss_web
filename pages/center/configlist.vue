<template>
	<scroll-view class="page">
		...
		<view v-if="showAddButton" style="height: 100%;width: 100%;padding: 10px;">
			<button class="cu-btn bg-green lg" style="display: block;margin:35%;line-height: 100px;height: 100px;width: 30%;" @click="AddConfig">点击添加</button>
		</view>

		<view class="cu-list menu" :class="[menuBorder ? 'sm-border' : '', menuCard ? 'card-menu margin-top' : '']" v-for="(item, index) in configList" :key="index">
			<view class="cu-item" :class="menuArrow ? 'arrow' : ''" @click="opendetail(item.id)">
				...
				<view class="cu-capsule radius">
					<view class="cu-tag bg-cyan " style="height: 60rpx;" @tap.native.stop="start(1)">开始</view>

					<view class="cu-tag bg-red " style="height: 60rpx;margin-left: 10rpx;" @tap.native.stop="stop(1)">结束</view>
				</view>
			</view>
		</view>
	</scroll-view>
